...
// SPINDLE_ENABLE_PORT ^= 1<<SPINDLE_ENABLE_BIT; // Debug: Used to time ISR
  ...
  // <!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
	...
	ARML_DIRECTION_PORT = (ARML_DIRECTION_PORT & ~ARML_DIRECTION_MASK) | (st.dir_outbits & (1<<ARML_DIRECTION_BIT))<<7;
	ARMR_DIRECTION_PORT = (ARMR_DIRECTION_PORT & ~ARMR_DIRECTION_MASK) | (st.dir_outbits & (1<<ARMR_DIRECTION_BIT));
	BASE_DIRECTION_PORT = (BASE_DIRECTION_PORT & ~BASE_DIRECTION_MASK) | (st.dir_outbits & (1<<BASE_DIRECTION_BIT))>>1;
  ...
  if (st.counter_x > st.exec_block->step_event_count) {
    st.step_outbits |= (1<<ARML_STEP_BIT);
    st.counter_x -= st.exec_block->step_event_count;
    if (st.exec_block->direction_bits & (1<<ARML_DIRECTION_BIT)) { sys.position[X_AXIS]--; }
  ...
  if (st.counter_y > st.exec_block->step_event_count) {
    st.step_outbits |= (1<<ARMR_STEP_BIT);
    st.counter_y -= st.exec_block->step_event_count;
    if (st.exec_block->direction_bits & (1<<ARMR_DIRECTION_BIT)) { sys.position[Y_AXIS]--; }
  ...
  if (st.counter_z > st.exec_block->step_event_count) {
    st.step_outbits |= (1<<BASE_STEP_BIT);
    st.counter_z -= st.exec_block->step_event_count;
    if (st.exec_block->direction_bits & (1<<BASE_DIRECTION_BIT)) { sys.position[Z_AXIS]--; }
...
ISR(TIMER0_OVF_vect)
{
  ...
  //STEP_PORT = (STEP_PORT & ~STEP_MASK) | (step_port_invert_mask & STEP_MASK); 
	// <<!!!!!!!!!!!!!!!!!!
	ARML_STEP_PORT = (ARML_STEP_PORT & ~ARML_STEP_MASK) | (step_port_invert_mask & (1<<ARML_STEP_BIT))<<6;
	ARMR_STEP_PORT = (ARMR_STEP_PORT & ~ARMR_STEP_MASK) | (step_port_invert_mask & (1<<ARMR_STEP_BIT))<<2;
	BASE_STEP_PORT = (BASE_STEP_PORT & ~BASE_STEP_MASK) | (step_port_invert_mask & (1<<BASE_STEP_BIT))>>2;
  ...
  { 
    ...
    // <!!!!!!!!!!!!!!!!!!!!!!!
	 	ARML_STEP_PORT = (st.step_bits & (1<<ARML_STEP_BIT))<<6;
		ARMR_STEP_PORT = (st.step_bits & (1<<ARMR_STEP_BIT))<<2;
		BASE_STEP_PORT = (st.step_bits & (1<<BASE_STEP_BIT))>>2;   
	}
#endif
...
void st_reset()
{
  ...
  //DIRECTION_PORT = (DIRECTION_PORT & ~DIRECTION_MASK) | dir_port_invert_mask;
	...
	ARML_DIRECTION_PORT = (ARML_DIRECTION_PORT & ~ARML_DIRECTION_MASK) | (dir_port_invert_mask & (1<<ARML_DIRECTION_BIT))<<7;
	ARMR_DIRECTION_PORT = (ARMR_DIRECTION_PORT & ~ARMR_DIRECTION_MASK) | (dir_port_invert_mask & (1<<ARMR_DIRECTION_BIT));
